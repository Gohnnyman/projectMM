...
/// is already pre-encoded, so the DMA reads it: no ISR refill, no real-time deadline, and therefore no
...
/// pad is not even mounted, and the defaults work. They exist for the LAPPING frontier
...
/// | `ci` | consumed items | descriptor nodes the mount used. Diagnostic. |
...
public:
    ...
    /// LCD_CAM-only, so the answer is "wherever this backend runs at all".
    ...
    /// Which of this backend's controls need the BUS rebuilt (not a re-encode) when they change:
    ...
    /// to drift apart: the base is the single home. Only `busDeinit` adds behavior of its own:
    ...
    /// self-test bit-verifies what the peripheral emitted: the one instrument that does not
    ...
    /// Did the bus come up as a ring? The orchestrator routes tick() on this, so it reports what
    ...
    static void MM_RAMFUNC ringEncodeTrampoline(void* user, uint8_t* dst, uint32_t firstRow, uint32_t rowCount,
        ...
        // Prefill only when the buffer's constants are gone (`needsPrefill`: the platform's
    ...
    void helperKick() {
        ...
        // Wait for the helper to be PARKED before notifying it for the next job. What keeps the
    ...
    void ensureSnapHelper() {
        ...
        // first kick doesn't wait on a park signal the helper only emits after it starts running.
