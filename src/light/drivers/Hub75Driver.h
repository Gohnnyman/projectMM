...
///
/// Every pin control starts UNSET, because a soldered line must never be guessed. These are the
/// maps for boards that wire a HUB75 connector, taken from each board's own published source
/// rather than from a tutorial. A panel's ribbon numbers its colour lines R1/G1/B1 (upper half)
...
public:
    ...
    // on an S3 exactly two GPIOs are spare after a 1/16 port, so a blind default would land on
    // octal-PSRAM, USB, UART0 or a strapping pin. A board's published map is a different matter,
    // and that is what these presets carry.
    ...
    /// this is the one control that trades colour precision against flicker. It is the user's call
    ...
    void defineDriverControls() override {
        ...
        // declared below the members it would set, and C++ initialises in declaration order, so a
        // constructor body's work was overwritten by their own default initialisers.
        //
        ...
        // The six colour lines and the address lines, in the order a HUB75 ribbon numbers them, so
        ...
        controls_.addControl("bitDepth", bitDepth, 2, 8);
    ...
    /// typo being corrected, the depth) are handled by prepare() re-initialising anyway.
    ...
    void prepare() override {
        ...
        // The correction scratch: one whole frame, because the encoder needs the packed frame
        // rather than one light at a time. Cold path, and grow-only across rebuilds.
        ensureWire(static_cast<size_t>(geo_.width) * geo_.height *
                   (correction_.outChannels ? correction_.outChannels : 3));
        if (!wire_) {
    ...
    void tick() MM_NONBLOCKING override {
        ...
        if (!wire_ || wireCap_ < static_cast<size_t>(lights) * outCh) return;   // not ready

        // Correct every light into the scratch, then encode the whole frame. The encoder reads a
        // packed RGB frame rather than working light-by-light, because a bit plane needs the pixel
        // at (x, r) and the one half a panel below it in the same pass.
        const uint8_t* src = sourceBuffer_->data();
        for (nrOfLightsType i = 0; i < lights; i++) {
            correction_.apply(src + static_cast<size_t>(i) * srcCh,
                              wire_ + static_cast<size_t>(i) * outCh, srcCh);
        }
    ...
    /// A correction change can alter outChannels, which is what the scratch is sized from: resize
    /// before the next tick reads it, the same rule RmtLedDriver follows.
    ...
    void tick1s() MM_NONBLOCKING override {
        ...
        if (hz == lastRefresh_) return;      // never re-serialise an unchanged value on the 1 Hz tick
        lastRefresh_ = hz;
...
private:
    ...
    static constexpr BoardPins kBoards[] = {
        ...
        {"MoonHub75",   1,  5,  6,   7, 13,  9,  16, 48, 47, 21, 38,  18,  8,  4, nullptr, false},
        // Adafruit MatrixPortal S3, from the board's own CircuitPython pins.c.
        {"MatrixPortal S3", 42, 41, 40,  38, 39, 37,  45, 36, 48, 35, 21,   2, 47, 14, nullptr, false},
        // Waveshare ESP32-S3-RGB-Matrix (SKU 34422), from WLED's WAVESHARE_S3_PINOUT constant.
        // Its gpio array is {R1,G1,B1,R2,G2,B2,A,B,C,D,E,LAT,OE,CLK} — LAT and OE come BEFORE CLK,
        // so reading it as ...CLK,LAT,OE swaps all three.
        {"Waveshare RGB Matrix", 4,  5,  6,   7, 15, 16,  18,  8,  3, 42,  9,  41, 40,  2, nullptr, false},
