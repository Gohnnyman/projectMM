...
// names the role of each one — the color roles a strip/panel needs (Red/Green/Blue/White)
...
// elsewhere). The color roles come first so a plain RGB(W) light only ever uses the low
...
// must NEVER be renumbered — a new one is appended within its group (color roles before the
// fixture roles) and existing values keep their index. White = a normal/cold white; WarmWhite is
// the second white a CCT fixture adds. Yellow/UV are the extra par-can colors (a 6-channel
