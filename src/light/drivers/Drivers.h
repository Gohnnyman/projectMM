...
/// that outputs the whole buffer (such as PreviewDriver) doesn't call it.
///
...
public:
    // Accepts output drivers only, so the "+ add" picker under Drivers offers the drivers: not
    ...
    ~Drivers() override {
        ...
        LivePalettes::clear(livePtrs_);
        MoonLivePalette::clearActiveInstance(&paletteScriptModule_);
        // Free the compiled script too, not detach the seam: release() may never run (a stack
    ...
    ///
    /// **ON is the better configuration: the switch exists to A/B it (and as an escape hatch),
    ...
    void refreshLivePalettes() {
        ...
        // script the user wrote: it keeps its place in the list and carries no chips, which
    ...
    // The brightness fed to the LUT: 0 when powered off, else the set brightness. Keeping
    ...
    void applyRelay() {
        ...
        // Release a pin the user cleared, or it stays asserted forever on a GPIO nothing owns.
    ...
    /// The pins driven last time, so clearing the list can release them. The control's text
    ...
    void prepare() override {
        ...
        // module the scheduler mounted, which is exactly the owner the seam wants.
        const uint8_t hadLive = liveCount_;
        refreshLivePalettes();
        LivePalettes::set(livePtrs_, liveTags_, liveCount_);
        // A CHANGED count needs the control rebuilt, not the seam republished: `palette`'s
        ...
        // drive. If it can't allocate (low memory) the split won't engage below and we fall
        ...
        // buffer allocated. Decided from the alloc OUTCOME (no if constexpr(hasPsram)): so a
    ...
    // right after and stops the task if the last driver left.
    //
    // If the worker does NOT come back within the timeout, the caller is about to DELETE the
...
private:
    ...
    void passBufferToDrivers() {
        // No active Layer (e.g. the last Layer was deleted): clear every
