...
/// pins, same controls as [I80LedDriver](I80LedDriver.md) — the difference is entirely underneath:
/// who programs the DMA.
///
...
/// class adds only what is i80-specific — the sacrificial WR/DC pins and the platform forwards — which
/// is why it is nearly all one-liners.
///
...
public:
    ...
    /// WR (pixel clock) and DC. The peripheral drives both and the WS2812 strands ignore both, but the
    /// bus needs them on real GPIOs. Peripheral-fixed rather than strand wiring, so a sensible
    /// overridable default cannot do harm.
    ///
    /// **`clockPin` does two jobs, and with a 74HCT595 expander the second is load-bearing:** WR
    /// toggles once per bus word in hardware, which is exactly what a '595's SRCLK needs — the pixel
    ...
    static constexpr bool kExactLaneCount = true;   // the bus is 8 or 16 lanes, never partial
    ...
    /// A data lane on WR or DC is fatal: the GPIO matrix would route two signals to one pin, and that
    /// lane would emit the clock/DC waveform instead of pixel data — silent corruption on that strand.
    const char* validateBusFatal() const {
        if (clockPin == dcPin) return "clockPin and dcPin must differ";
        if (shiftMode() && latchPin >= 0) {
            if (latchPin == clockPin) return "latchPin is on clockPin (WR) — the latch needs its own GPIO";
            if (latchPin == dcPin)    return "latchPin is on dcPin — the latch needs its own GPIO";
        }
        return nullptr;
    }
    const char* validateBusPins(const uint16_t* lanes, uint8_t n) const {
        for (uint8_t i = 0; i < n; i++) {
            ...
            if (lanes[i] == static_cast<uint16_t>(dcPin))    return "a data pin is on dcPin";
        }
        return nullptr;
    }
    ...
    /// WR/DC are part of the bus identity, so a change to either rebuilds it — not just a data-pin edit.
    void recordBusPins() { lastClockPin_ = clockPin; lastDcPin_ = dcPin; }
    bool extraBusPinsCurrent() const { return lastClockPin_ == clockPin && lastDcPin_ == dcPin; }
