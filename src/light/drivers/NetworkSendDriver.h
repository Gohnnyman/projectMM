...
///
/// UDP is fire-and-forget, so a dead receiver is invisible to the sender: the send loop
...
public:
    ...
    /// per-lane encode (which reads laneCounts_).
    ...
    void tick() MM_NONBLOCKING override {
        ...
        // whole buffer from start), so it covers its lights: and a frame
        ...
        // (Harmless for a 3-channel strip, where the partial pixel is a pixel; corrupting for
...
private:
    ...
    void resizeCorrected() {
        ...
        // Size for the window slice this sender transmits, not the whole
