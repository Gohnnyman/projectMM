...
struct Correction {
    ...
    /// and both are labeled for what they do.
    ...
    // Refresh the brightness LUT. Split out so a brightness-only change re-scales the LUT
    ...
    void rebuildBrightness(uint8_t brightness) {
        for (int v = 0; v < 256; v++) {
            ...
            // fade-out SNAPS off partway down and the dimmest usable settings are missing.
    ...
    /// A color role the light doesn't carry (offset == kAbsent) is not written, so a
    /// wiring that omits, say, red doesn't emit it.
    ///
    ...
    inline void apply(const uint8_t* src, uint8_t* out, uint8_t srcChannels) const {
        ...
        // dimmer sits at 0 is dark. Written every frame like any other role, so a preset
