...
public:
    ...
    /// is for the preset-driven channel-count change that doesn't rebuild structure.
...
protected:
    ...
    /// the standards answer (CIE 15 / ISO 11664-4) and models the thing being corrected,
    ...
    /// is named for what it does rather than presented as equivalent.
    ...
    void defineCorrectionControls() {
        ...
        // above is the user-facing control; presetRef_ carries the reference across a reboot.
...
private:
    ...
    void buildPresetOptions() {
        ...
        //      the user chose it. Adopt the Select's id. (This rebuild runs BEFORE
...
protected:
    ...
    /// onboard status LED at index 0, the main strip from index 1) sets its
    ...
    // see what a driver consumes, instead of guessing from grid × pins. `channels` is the
