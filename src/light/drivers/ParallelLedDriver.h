...
public:
    ...
    /// builds is DERIVED from the configured pin count: ≤8 pins → an 8-bit bus (uint8
    ...
    /// so the derived sets them after construction; the base declares them. Both backends take
    ...
    /// into background DMA).
    ///
    /// **ON is the better configuration: the switch exists to A/B it, not because some setups
    ...
    void onControlChanged(const char* name) override {
        ...
        // off peripheralSel_ (the newly written index): so the live backend was ALREADY swapped during the
    ...
    void prepare() override {
        // Drain any in-flight transfer BEFORE parseConfig, not before reinit's rebuild. parseConfig
    ...
    /// bus: but ONLY when the frame size changed. Skipped while (effectively) disabled (would
    ...
    /// *Robustness* rule (the architecture) says a bad input
    ...
    bool busGaveUp() {
        ...
        if (++giveUpRetry_ >= kGiveUpRetryTicks) {
            ...
            // re-arms the bus, which is what recovers it.
    ...
    // no-op on an idle buffer). If a wait times out here we cannot skip: the buffers are about
    ...
    //
    // **A ROW RANGE, not always the whole frame**: the whole-frame call is the range [0, all).
    ...
    void MM_RAMFUNC encodeRows(uint8_t outCh, uint8_t* dst,
        ...
        // byte is presented and every strand idles LOW through the pad: the WS2812 reset.
    ...
    void encodeLoopbackFrameShift(uint8_t* frame, const uint8_t* wire, uint8_t outCh,
                                  nrOfLightsType lights) {
        auto* out = reinterpret_cast<Slot*>(frame);
        // The pattern rides `loopbackStrand`: whichever '595 output the jumper is on. It
...
public:
    ...
    /// DriverBase). Gated on inited_: a driver reports its block only while it holds the bus,
...
protected:
    ...
    /// QuinLED Dig-2-Go showed 512 bytes for this driver against RmtLedDriver's 32 KB while
    ...
    // is 8 or 16 bits wide however many pins the board drives:
    ...
    // Kept in the base (one owner of the latch + the padding), so both derived busInit()s pass
    // these two calls.
public:
    ...
    /// rejects an NC data pin and parks them on WR, while MoonI80 routes its own GPIOs and does
    ...
    // counts are equal, so this is the original behavior.) A backend's encode trampoline
...
protected:
    ...
    void prepareWire(uint8_t outCh) {
        ...
        // reads. Grows-only, so a direct-mode driver that later enables the expander grows.
    ...
    bool parseConfig() {
        ...
        // WR/DC candidate: only the lanes that drive a strand would show
        ...
        physPins_ = n;
        laneCount_ = lanes;
        // The latch takes the bus bit above the data pins (data pins are bus bits 0..n-1,
        ...
        setConfigWarn(warn);
        // With nothing more urgent to show AND lights driven, report how many
    ...
    void reinit() {
        ...
        // internal-RAM-only) cannot allocate a frame larger than its internal DMA block: and on
        ...
        if (const size_t budget = peripheral_->dmaBudgetBytes();
            ...
            // deinit() above already cleared the bus and inited_: report and bail.
    ...
    void runLoopbackSelfTest() {
        ...
        // the render loop sends. Heap alloc is fine: control-driven, off
        // the hot path.
        ...
        // would build the bus on the wrong GPIO, so ignore the override (busPinList() copies
