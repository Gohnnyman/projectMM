...
**System variables cost three byte stores per tick.** A script reads `width`/`height`/`depth` (and a modifier `x`/`y`/`z`) as arena slots the binding refreshes before `run()` — a null check and a byte store each, replacing nothing, so the per-tick figure above is unchanged by them. `t` costs nothing at all: it is an argument register the host already passes. The compile path grew (a system-variable table, resolved before locals and controls) but that is cold-path, once per `source` edit.
