...
| **LCD_CAM 16-lane** | ESP32-S3 (SE 16 V1 + LightCrafter 16, n8r8) | SE16 data `47,48,21,38,14,39,13,40,12,41,11,42,10,2,3,1` · WR/DC `5`/`6`; LC16 data `47,21,14,9,8,16,15,7,1,2,42,41,40,39,38,48` · WR/DC ghost `33`/`34` | **Reaches the full 16384 lights — the 16K target — where Parlio caps at 4096.** SE16 16-lane doubling sweep (128×128 grid, 2026-07-12): 512 → 4208 µs, 1024 → 7089 µs, 2048 → 12778 µs, 4096 → 21945 µs, 8192 → 40570 µs, **16384 → 76979 µs (~13 fps)**. Scales linearly at **~4.6 µs/light** (≈2× Parlio's ~2.7 µs — the i80 bus + 16-bit slot path is heavier per light). The `LcdLed` status reports the live count (`driving N of 16384 lights`). | **No contiguous-block ceiling — the key difference from Parlio.** LCD_CAM allocates its DMA buffer via `esp_lcd_i80_alloc_draw_buffer` **from PSRAM**, so it isn't bound by the ~368 KB largest-internal-block limit that caps Parlio at 4096 lights; it drives all 16384. **But 16K is reachable, not *animatable* on one core:** 1024 LEDs/lane → ~13 fps (a slideshow), exactly the deep-per-lane wall (§ Step 3, [multicore top-down](backlog/multicore-analysis-top-down.md)). And the ~19–77 ms encode runs on **core 0**, which on the LC16 **starves the W5500 SPI-Ethernet** (also core 0) → link drops, HTTP times out while the render loop keeps ticking. This is the measured contention that justifies the [multicore pipeline (Step 2)](backlog/multicore-analysis-top-down.md#step-2--multicore-effects-on-core-0-encodetransmit-on-core-1-the-pipeline--shape-decided) on classic/S3 — a core-budget limit, not a fault. |
| **Parlio 16-lane** | ESP32-P4 (testbench, n16r8) | 16 data pins `21,20,22,23,24,25,26,27,32,33,39,40,41,42,43,44` | 16-lane doubling sweep (`ledsPerPin` 32→256/pin on a 128×128 grid, 2026-07-12; reproduced within 0.3% on a second P4). Tick scales **linearly** with lights: 512 → 1653 µs, 1024 → 2925 µs, 2048 → 5514 µs, **4096 → 10760 µs** (~93 fps). The `ParlioLed` status reports the live count (`driving N of 16384 lights`). **Why 93 fps, not the ~125 fps wire ceiling** (256 LEDs/lane × 24 bits × 1.25 µs + 300 µs latch = 7980 µs of WS2812 wire time): the tick is `encode (CPU) + transmit (wire)` run **serially**. Subtracting wire time from tick isolates the encode at a constant **~0.68 µs/light** (393 / 705 / 1374 / **2780 µs** across the sweep) — non-overlapped CPU that pipelining (double-buffer, or the multicore encode-on-core-1 split) would hide behind the DMA, recovering the wire ceiling. This is a second quantified argument for the [multicore pipeline](backlog/multicore-analysis-top-down.md): ~26% of the 256/pin frame budget is serial encode. | **Single-DMA ceiling ≈ 4096 lights (256/pin).** 512/pin (8192) → `Parlio init failed — check pins / memory`. The P4 has 33 MB free heap but the largest *contiguous* internal block is ~368 KB, and the 16-bit single-shot DMA buffer needs one contiguous block — so it's a **contiguous-block limit, not total memory** (it bites well before the 65535-byte/lane byte cap). Reaching the full 16384 (1024/pin) needs the [Parlio chunked-transfer](backlog/backlog-light.md#led-drivers--deferred) work (frame split across DMA bursts). |
