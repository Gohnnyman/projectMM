...
Nothing to configure — HA discovers the device automatically. Two paths (both zero-config; you can leave both on):

- **MQTT auto-discovery** (default, uses the Mosquitto add-on): with `haDiscovery` on, the device announces itself on the retained `homeassistant/light/…/config` topic, and HA **auto-creates a light entity** named after the device with **on/off + brightness** (the discovery config declares `brightness` only, so the entity has no colour control); it greys out when the device drops offline. Colour/palette control stays on the separate `hsv/set` topic — not the auto-created entity.
- **WLED integration** (works with no MQTT at all): HA's built-in WLED integration discovers the device over zeroconf and reads the WLED-compatible `/json` API projectMM already serves — on/off + brightness work without a broker.

**Why keep both on?** The two entities differ in what they expose. The MQTT-discovery entity is **on/off + brightness** (its discovery config declares `brightness` only). The WLED entity is **on/off + brightness** too, plus WLED's effects/palettes list — which surfaces only projectMM's placeholder entries, because the real effect pipeline (Layer → Effect) lives in projectMM's own UI, not WLED's flat model. Neither auto-created entity carries a colour picker; colour/palette control is the separate `hsv/set` topic. What actually differs is transport and reachability: MQTT **pushes** state (millisecond latency, retained across offline periods, works across VLANs/guest nets where zeroconf drops) and lets projectMM sit next to other MQTT devices (Tasmota, ESPHome, Zigbee2MQTT) with one convention; the WLED path is **zero-infrastructure** (no broker) and drops the device straight into an existing WLED workflow. On a simple flat LAN with only projectMM devices, WLED alone is fine; anything else — a segmented network, other MQTT devices, sub-second automations — is where MQTT earns its slot. Richer per-control publishing (per-Layer, per-effect) is a natural fit for the MQTT path, since WLED's fixed schema can't host it.
