...
- **MQTT auto-discovery** (default, uses the Mosquitto add-on): with `haDiscovery` on, the device announces itself on the retained `homeassistant/light/…/config` topic, and HA **auto-creates a light entity** named after the device. On/off, brightness, and the HSV colour wheel are wired up; the entity greys out when the device drops offline.
