...
- **`MoonModule::quiesce()`** — core calls it on the parent before every structural mutation (`addChild` / `removeChild` / `replaceChildAt`), because a mutation frees or reallocates memory the worker may be walking. See [§ the prepare pass](#control-changes-the-prepare-pass) above.
