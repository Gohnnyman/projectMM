...
**Graceful degradation under transport backpressure.** The preview is the transport-side sibling of the memory-side [§ Degradation cascade](#degradation-cascade): when the link can't keep up with a full-resolution frame (128² = ~49 KB), quality is shed rather than the loop stalled, in video-streaming order, frame rate first, then resolution. Frame rate self-limits at the device: a new frame starts only when the previous one drained, so a slow link simply gets fewer whole frames (each WebSocket message is atomic; a client blocked past a wall-clock budget is closed and reconnects, a blip, not a freeze). Resolution is **receiver-driven** (the adaptive-bitrate idea behind HLS/DASH): the browser measures the frame rate that actually arrives, the one true end-to-end number, which the sender's socket can never see, and requests a coarser or finer spatial lattice via the `/wsp` uplink; the device serves the coarsest standing request, and no standing request means full detail. A coarsening the receiver keeps must *pay* in measured rate, so a render-bound source never loses detail for nothing. The controller is one pure function in `preview-adapt.js` with its own unit tests; the device side is `PreviewDriver` + `HttpServerModule`, payload-agnostic, so other bulky streams can ride the same transport.
