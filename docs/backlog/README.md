...
- [multicore-analysis-bottom-up.md](multicore-analysis-bottom-up.md) — multicore & driver scaling, Stage-1 survey. Combines MoonLight's documented two-task architecture with **hardware measurement on the P4 at 16384 lights**: the surprise is that the driver cost is the CPU **WS2812 encode (~24 ms, 85%)**, not the DMA wait (~0). Gives the per-driver max-lights-per-pin (RMT/LCD have no hard cap — fps/memory only; Parlio hits a 65535-byte/lane hardware single-transfer ceiling (897 RGB lights)), documents two driver bugs found + fixed (Parlio over-limit silent-fail, bus-not-rebuilt-on-shrink), and re-aims multicore at parallelising the encode. Consolidates the scattered multicore notes; records frame-pacing-decided-against.
- [multicore-analysis-top-down.md](multicore-analysis-top-down.md) — the Stage-2 build plan on those findings: the ordered steps — (1) encode optimisation (skip-unchanged → bit→slot lookup → SIMD, one core, the prerequisite), (2) multicore encode+transmit on core 1 with a double-buffer + notification handoff (gated on measured flicker), (3) the ceiling raises — where the fps wall (~1024 LEDs/lane at 30 µs/light) makes **lanes, not per-lane depth**, the real lever: 16-lane widening then a shift-register virtual driver multiply the *animated* total, while Parlio chunking is marginal (only closes the ~897→1024 band) — with the dependency graph (encode-opt unblocks/obviates the rest) and the settled non-steps (frame pacing, the DMA-wait).
