...
- **Building from source on Debian 12 / Raspberry Pi OS hits GCC false positives** — the shipped Linux `.deb` and tarball are prebuilt x86-64 binaries, so a user downloading them never compiles. Building *from source* on a stock Debian 12 (GCC 12) or Ubuntu 24.04 (GCC 13) does, and those versions fire `-Wnull-dereference`, `-Wrestrict` and `-Wstringop-overflow` inside libstdc++ and glibc headers, which `-Werror` makes fatal; four of the twelve are in `<bits/stl_algobase.h>`, `<streambuf>` and `<stdio2.h>`, so no source change reaches them. CI pins `g++-14` for the release binary, and the local gate uses whatever GCC is installed (16 on the maintainer's Mac), so nothing exercises 12 or 13. Fix by not making warnings fatal outside CI, so a from-source build warns instead of failing.
