...
### Centralize the enabled-gate for setup/onBuildState in the Scheduler (backlog)

**Problem.** The Scheduler already owns the enabled-gate for the **loop** callbacks — one predicate (`!respectsEnabled() || enabled()`) applied in `Scheduler::tick` for top-level modules and in `MoonModule::tickChildren` for children, so a driver's `loop()` never checks `enabled()` itself. But the **setup / build** path has no such gate: `Scheduler::setup` (Phase 3 `setup()`, Phase 4 `onBuildState()`) and `Scheduler::buildState()` call **every** module regardless of `enabled()`. So a persisted-disabled module still runs its acquire at boot and on every tree rebuild. The disable-releases-resources work papered over this by having each resource-holding module **self-gate**: `if (enabled())` now appears in ~9 sites across 5 files (`setup`, `onBuildState`, `onCorrectionChanged`, `setSourceBuffer`, `setCorrection` in RmtLed/Parlio/AudioService/NetworkSend, plus the DevicesModule `active_` election). That's the same rule the Scheduler *already* applies to loops, re-implemented by hand per module and inconsistently (some `if(!enabled()) return;`, some `if(enabled()) reinit();`). A catalog module author now has to remember to gate their acquire — exactly the per-module bookkeeping *Complexity lives in core* says to push down.

**The clean design.** Core owns the gate for setup/build the same way it owns it for loops, so modules keep plain unconditional `setup()`/`onBuildState()` that just acquire:
- `Scheduler::setup` Phase 3/4 and `Scheduler::buildState()` gate each top-level module with the existing `shouldRun` predicate (`!respectsEnabled() || enabled()`).
- `MoonModule::onBuildState`'s child recursion (and the container `setup()` recursion) gate children the same way `tickChildren` already does — factor the gate into one shared helper so the rule lives in exactly one place, mirroring `tickChildren`.
- The transition path (`onEnabled(true) → setup()` / `releaseOnDisable`) already re-acquires on enable and stays as-is.

Result: `RmtLedDriver::setup()` is `{ parseConfig(); reinit(); }` again, `onBuildState()` drops its guard, and the ~9 scattered checks collapse to ~2 central gates. A new driver's `setup()` acquires unconditionally and **never mentions `enabled()`** — core guarantees it isn't called when disabled.

**The subtlety that makes it a real design task, not a mechanical move.** Today `parseConfig()` (safe when disabled — fills members only) and `reinit()` (the acquire) are fused in one `setup()`; a whole-method central gate skips *both*, which is fine for the resource-holders but must be **audited against every module's `setup()`** for must-run-even-if-disabled work. Known case: `DevicesModule::setup()` restores the persisted device list — a disabled instance should still show its cached rows, so its list restore must run even when the acquire is gated. So the central gate may need to distinguish "always-run init" from "gated acquire," or those modules split their `setup()` accordingly. That audit + the split decision is the work, and it changes the contract for *every* module's setup — a core-orchestrator change, so it earns a `/plan` and the *Refactor for simplicity* alternatives write-up (whole-method gate vs. split init/acquire vs. an explicit `acquire()`/`release()` hook pair the base calls).

**Why it's a separate commit, not folded into the fix that exposed it.** The self-gating patches are correct and hardware-verified (the P4 ParlioLed-dark-at-boot bug is fixed, three regression tests pin it). Those tests assert *behavior* (a disabled module doesn't acquire), not *mechanism*, so they transfer unchanged and become the safety net when the gate moves to core. Bundling an orchestrator-contract refactor into a targeted bug fix mixes concerns and makes the diff hard to review/bisect — ship the fix, then do this cleanly.

**Related:** the `~AudioService`/`~DevicesModule` destructor vacates shipped with the fix (a bare destruction must not leave the `active_` static dangling) are independent of where the enabled-gate lives — keep them regardless.
...
### PinsModule — one place that coordinates GPIO assignment (backlog)

**Problem.** Pin assignments are declared **all over the tree** today — `RmtLedDriver.pins`, `AudioService.sckPin/wsPin/sdPin`, `NetworkModule.ethMdcGpio/…`, `I2cScanModule.sda/scl`, `IrService.pin`, and each board's catalog entry sets them independently. There is no single view of *which pin does what on this device*, no owner that arbitrates, and (until the [pin-uniqueness check](#pin-uniqueness-check-across-modules-prevents-conflicts-replaces-a-singleton-hack) lands) nothing stops two modules claiming the same GPIO. A user wiring a board has to open every module to reconstruct the pin map in their head.

**Idea.** A read-only-first **PinsModule** (a fixed System module, like [TasksModule](../moonmodules/core/system.md#tasks) / I2cScan — wired-by-code, always present) that presents the **one coordinated pin map**: enumerate every `ControlType::Pin` control across the tree (the mechanism the pin-uniqueness check already needs — a `Pin` value of `-1` is unused, any other value is a claim, list-pins like `"18,19,20"` expand), and show each claimed GPIO with *who owns it* and *what role* (BCLK / data / MDC / LED lane / …). Cross-reference [docs/reference/gpio-usage.md](../reference/gpio-usage.md) so a claim on a reserved/strap pin is flagged. This is the "what runs where" of pins, the sibling of TasksModule's "what runs where" of tasks.

**Why a module, and why now.** Phase 1 is a **read-only** map — it *shows* the pin picture the [pin-uniqueness check](#pin-uniqueness-check-across-modules-prevents-conflicts-replaces-a-singleton-hack) already computes, without owning or enforcing anything. *Later* (the staged authority increment below) it can become the natural home for the pin-conflict arbitration that check currently scatters across two HTTP sites (`POST /api/modules` + `POST /api/control`): the PinsModule would own the live pin map and *be* the authority both consult. Later still it enables live pin reassignment (the "swap two drivers' pins" case the uniqueness item flags as needing a free intermediate — a coordinator can broker the swap), and it pairs with [disabling-releases-resources](#disabling-a-module-should-release-its-resources-not-just-stop-its-loop-backlog) (a disabled module's pins show as freed). The read-only map earns its place on its own; the authority role is a deliberately-later increment, not a phase-1 responsibility.

**Scope, staged (concrete-first, like TasksModule).** (1) read-only pin map from the live `Pin` controls; (2) reserved/strap flagging against the gpio-usage reference; (3) the conflict authority the uniqueness check calls; (4, later) live reassignment / broker. Domain-neutral — it reads `ControlType::Pin`, knows nothing about lights. **Open question:** does the map need a per-role label (BCLK vs data vs LED) — which means `Pin` controls carrying a role hint — or is the owning control's *name* (`sckPin`, `pins`) enough? Decide when specced. This is a **spec + /plan** item, not a quick add.

**Two complementary angles — study the field, build our own.** There are two distinct questions about a pin: *who owns it and why* (the **static/ownership** map — which module claims GPIO18, for what role) and *what is it doing right now* (the **dynamic/live-state** view — high / PWM / analog, animated on a board diagram). PinsModule is the ownership map, read from the projectMM `Pin` controls; a live-state layer is a separate, later option. For **both**, survey how the field solves it — real-time GPIO web viewers for the live angle, and how **Tasmota** (its Template / GPIO-role assignment UI) and **ESPHome** (its `pin:` schema + substitutions) present pin *ownership* for the static angle — then write our own fresh against projectMM's `ControlType::Pin` model per [*Industry standards, our own code*](../../CLAUDE.md#principles) (learn the ideas, credit the survey in a `history/` digest, don't depend on or trace any one tool). The ownership map is the part no external tool gives us — it falls out of our own control model.
