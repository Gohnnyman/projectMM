...
**Problem.** Pin assignments are declared **all over the tree** today — `RmtLedDriver.pins`, `AudioModule.sckPin/wsPin/sdPin`, `NetworkModule.ethMdcGpio/…`, `I2cScanModule.sda/scl`, `IrModule.pin`, and each board's catalog entry sets them independently. There is no single view of *which pin does what on this device*, no owner that arbitrates, and (until the [pin-uniqueness check](#pin-uniqueness-check-across-modules-prevents-conflicts-replaces-a-singleton-hack) lands) nothing stops two modules claiming the same GPIO. A user wiring a board has to open every module to reconstruct the pin map in their head.

**Idea.** A read-only-first **PinsModule** (a System Peripheral, like [TasksModule](../moonmodules/core/services.md#tasks) / I2cScan) that presents the **one coordinated pin map**: enumerate every `ControlType::Pin` control across the tree (the mechanism the pin-uniqueness check already needs — a `Pin` value of `-1` is unused, any other value is a claim, list-pins like `"18,19,20"` expand), and show each claimed GPIO with *who owns it* and *what role* (BCLK / data / MDC / LED lane / …). Cross-reference [docs/reference/gpio-usage.md](../reference/gpio-usage.md) so a claim on a reserved/strap pin is flagged. This is the "what runs where" of pins, the sibling of TasksModule's "what runs where" of tasks.

**Why a module, and why now.** It is the natural home for the pin-conflict arbitration that the [pin-uniqueness check](#pin-uniqueness-check-across-modules-prevents-conflicts-replaces-a-singleton-hack) currently scatters across two HTTP sites: instead of the check living in `POST /api/modules` + `POST /api/control`, the PinsModule owns the live pin map and *is* the authority both consult. Later it enables live pin reassignment (the "swap two drivers' pins" case the uniqueness item flags as needing a free intermediate — a coordinator can broker the swap), and it pairs with [disabling-releases-resources](#disabling-a-module-should-release-its-resources-not-just-stop-its-loop-backlog) (a disabled module's pins show as freed).
