...
- `mAPerColorChannel` / `mAPerWhiteChannel`: what one channel draws at full, for that estimate. Per **channel**, not per light: a white die draws about twice a colour one, so a single per-light figure under-reports white-heavy frames, which is the direction that browns out a supply. Defaults (8 and 16) are measured on a 5 m SK6812 RGBW strip. Shown only on drivers that price their frames.
