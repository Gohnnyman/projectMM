...
- **One umbrella include per light-domain kind.** A new module includes a single umbrella header — `light/effects/Effect.h`, `light/layouts/Layout.h`, `light/modifiers/Modifier.h`, or `light/drivers/Driver.h` — which pulls in the base class plus that kind's common helpers (for effects: `Layer` for the render-context accessors, `draw`, `Palette`, `math8`, `color`). So a module starts with one `#include` and the copy-edit-register workflow has no include guesswork. Two constraints the umbrella respects: (a) it includes the *kind's* real common set, not a blanket one — a driver umbrella is mostly `DriverBase` (which already bundles `Layer`/`Buffer`/`Correction`/platform), a modifier umbrella is `ModifierBase` + integer trig; (b) it can't be the base class itself — `EffectBase.h` forward-declares `Layer` (whose accessors are defined in `Layer.h` after `Layer` is complete), so folding `Layer.h` into `EffectBase.h` would be circular; the umbrella is a *separate* thin header that depends on both, which neither base does. A module needing a helper outside its umbrella's set just adds that one extra `#include`. **New modules use the umbrella from the start; existing modules migrate to it as a mechanical sweep — folded into the [hook-rename](history/plans/) sweep so each file is touched once, not twice.** (`RainbowEffect` is migrated as the proof; the rest follow.)
