...
- **Not in this change** (tracked, not lost): the encode's remaining 46 → 21.6 µs gap (the emit loop; the correction pass), the lying fps header, `_DMA_EXTENSTION` (costs the RAM we lack — see `docs/backlog/shift-register-driver-analysis.md` § 7.6).
