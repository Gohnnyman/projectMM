...
The "hardware half" of release-on-disable ([backlog: Disabling a module should release its resources](../../backlog/backlog-core.md#disabling-a-module-should-release-its-resources-not-just-stop-its-loop-backlog)). The "display half" shipped with the pin map (a disabled module's pins already drop from the map, `PinsModule.h` `collect()` gates on `!respectsEnabled() || enabled()`). This makes that display *truthful*: a disabled module actually gives its peripheral/socket back, so the freed GPIO is really reusable — the prerequisite for pins increment #5 (live reassignment). PO-directed.
