...
`moondeck/build/improv_provision.py` so a one-click MoonDeck button can
...
Primary credentials source: `moondeck/moondeck.json` under the active
...
def _moondeck_active_network_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    moondeck/moondeck.json. Returns (None, None) when moondeck.json is absent,
...
def get_host_wifi() -> Tuple[Optional[str], Optional[str]]:
    ...
    Resolution order:
      1. `moondeck/moondeck.json` active network's `wifi` block — the primary
