...
/// Pins the string→int conversion the flat readers share (mm::json::parseIntStr, and parseInt through it). The interesting half is what a NON-number does: every caller treats the result as a value to clamp or compare, so "not a number" has to arrive as the fallback rather than as a plausible-looking integer.
...
TEST_CASE("a value too large to represent reads as the fallback instead of wrapping") {
    // The case that made this worth sharing: atoi on an out-of-range value is undefined behavior, and a caller narrowing the result would store a DIFFERENT valid number, a Hue id of "65537" becoming light 1. Out-of-range must be visible, not silently wrapped.
    ...
    // ONE past each boundary is the case that separates the two overflow checks, and the one that differs by target: where `long` is 64-bit (desktop) these fit `long` and only the INT_MAX compare rejects them; where it is 32-bit (ESP32, Windows) strtol saturates and sets ERANGE. Both must reach the fallback, or a Hue id of "2147483648" would land as INT_MAX.
