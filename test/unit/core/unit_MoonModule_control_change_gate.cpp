...
// the pipeline only when `controlChangeTriggersPrepare()` returns true.
...
TEST_CASE("controlChangeTriggersPrepare: Layout and Modifier opt in") {
    mm::GridLayout layout;
    mm::MultiplyModifier modifier;
    CHECK(layout.controlChangeTriggersPrepare("width"));
    CHECK(modifier.controlChangeTriggersPrepare("mirrorX"));
}
...
TEST_CASE("controlChangeTriggersPrepare: effects and Drivers do NOT rebuild") {
    mm::NoiseEffect effect;
    mm::Drivers drivers;
    CHECK_FALSE(effect.controlChangeTriggersPrepare("scale"));
    CHECK_FALSE(drivers.controlChangeTriggersPrepare("brightness"));
}
