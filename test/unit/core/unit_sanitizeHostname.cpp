...
/// Pins mm::sanitizeHostname (core/Control.h), the RFC-1123 coercion that keeps the device name a valid DNS/mDNS hostname. deviceName is the single network identity, the mDNS <name>.local, the SoftAP SSID, and the DHCP hostname all derive from it, so an invalid name (spaces, punctuation) would break mDNS registration and the installer's clickable .local link. These tests lock the rule: keep [A-Za-z0-9-], collapse any run of other chars to a single '-', trim leading/trailing '-', idempotent on an already-valid name, empty stays empty (caller supplies the MAC fallback).
