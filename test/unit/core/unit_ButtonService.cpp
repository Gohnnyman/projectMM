...
/// Pins the button service's two halves: the debounce state machine, and the mapping row that turns a settled edge into a control write.
/// The GPIO seam is a host stub whose level a test injects (platform::setTestGpioLevel), so a press is expressed as "the pin reads low for long enough", which is exactly what the module sees on a board.
...
struct FakeDrivers : public MoonModule {
    ...
    // A control WIDER than a byte, and one that goes negative. The surface reads both as a clamped byte, which is right for a fader and wrong for arithmetic on the control itself.
    uint16_t rate = 300;
    int16_t offset = -50;
...
// Stands in for a module carrying a pad grid (the control surface's presets). Only the parts a pad target touches: rows that publish a `slot`, and an `activate` field that fires one.
...
struct Rig {
    ...
    /// Active-low, so a pressed switch reads LOW.
...
TEST_CASE("a target round-trips through the type and number the editor shows") {
    // The stored form is one Module.control string; type and number are how a user edits it. A bug in either direction silently retargets a row, which is invisible until the button does the wrong thing, so both directions are pinned here.
    ...
    // A target this vocabulary cannot express reads back as unassigned, so the dropdown shows "(none)" while the row keeps working.
    // Rewriting it to fit would lose what the user set.
    // The editor offers the SURFACE only, because switch1 already targets Drivers.on and fader1 targets Drivers.brightness: offering those directly would be two paths to one place.
...
TEST_CASE("editing the type or the number re-composes the target, so they cannot disagree") {
    ...
    // The API's string form still reaches ANY control, which is the escape hatch for anything the surface does not carry: the editor is a convenience, not the only way in.
