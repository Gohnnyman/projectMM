...
// MirrorModifier folds the far half of the logical box back onto the near half per axis. modifyLogicalSize halves each mirrored axis (rounding up: ceil(size/2) = (size+1)/2, so an odd extent keeps its unpaired center column); modifyLogical then reflects any coordinate at or past the half-extent back via `half*2 - 1 - pos`, so a far-half physical light and its near-half mirror share one logical source.
...
// Fold a physical coord to its logical source. `box` is the physical box, run through modifyLogicalSize first so the modifier stashes its halved box for the const fold.
...
// modifyLogicalSize halves each mirrored axis, rounding up: an even 128 → 64, an odd 65 → 33 (the center column stays unpaired).
...
// A coord and its mirror across the box center map to the same logical position: on an even 8-wide axis, physical 7 folds to 0, 6 to 1, 5 to 2, 4 to 3, while the near half 0..3 passes through unchanged.
...
// An odd extent keeps its center column unpaired: on a 5-wide axis the half-extent is 3, so 0,1,2 pass through and the far edge 4→1, 3→2, leaving logical column 2 unpaired.
