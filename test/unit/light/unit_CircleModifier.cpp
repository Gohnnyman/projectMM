...
// CircleModifier folds every physical light to its Euclidean distance from the box center: a coord becomes (0, distance, 0), so a 2D box collapses to a single column of concentric rings. modifyLogicalSize runs the box's own far corner through that same fold and then grows every axis by one. Integer center offsets (box/2), float distance truncated back to lengthType, MoonLight's exact geometry.
...
// The box center folds to the origin (distance 0), and every coord collapses onto the single x=0/z=0 column, the box becomes a 1D radial run.
...
// A light's ring is its integer-truncated Euclidean distance from the center, so two lights equidistant from the center map to the SAME radius, the defining circle property. Center (4,4): (7,4) and (4,7) are both 3 away; (7,7) is sqrt(18)→4.
...
// modifyLogicalSize folds the far corner to its distance, then grows every axis by one: the logical box is a (radius+1)-tall column with x=1 and z=1. Corner (8,8,1) off center (4,4,0) is sqrt(16+16+1)=5.74→5, so the size is (0+1, 5+1, 0+1).
