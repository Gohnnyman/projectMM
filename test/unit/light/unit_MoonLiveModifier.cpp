...
TEST_CASE("a scripted modifier mirrors the pattern, the way a hand-written one would") {
    ...
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, width - 1 - xPos, yPos, zPos);"), 10, 20, 0);
...
TEST_CASE("the script reads the light's own position, not a fixed value") {
    ...
    const Coord3D a = transform(mmScriptAs("modifyLogical", "setXYZ(0, xPos, yPos, zPos);"), 7, 3, 1);
    CHECK(a.x == 7);
    CHECK(a.y == 3);
    CHECK(a.z == 1);

    const Coord3D b = transform(mmScriptAs("modifyLogical", "setXYZ(0, xPos, yPos, zPos);"), 200, 100, 2);
    CHECK(b.x == 200);
    CHECK(b.y == 100);
    CHECK(b.z == 2);
}
...
TEST_CASE("a script can swap axes, which is a transform no control could express") {
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, yPos, xPos, zPos);"), 5, 60, 0);
    CHECK(p.x == 60);
    CHECK(p.y == 5);
}

TEST_CASE("a script can offset a coordinate, the scroll a modifier usually hard-codes") {
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, xPos + 4, yPos, zPos);"), 10, 10, 0);
    CHECK(p.x == 14);
}
...
TEST_CASE("a broken script leaves the pattern alone rather than taking the layer down") {
    ...
    MoonLiveModifier m;
    m.defineControls();
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, xPos, yPos")));     // no closing paren, no semicolon
    m.prepare();
...
TEST_CASE("a coordinate too large for a script input passes through untransformed") {
    ...
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, 255 - xPos, yPos, zPos);"), 300, 10, 0);
...
TEST_CASE("editing the script changes the transform without a rebuild of the firmware") {
    ...
    MoonLiveModifier m;
    m.defineControls();
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, width - 1 - xPos, yPos, zPos);")));
    m.prepare();
    Coord3D box{255, 255, 1};
    ...
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, xPos, yPos, zPos);")));
    m.prepare();
...
TEST_CASE("a script computes with the usual precedence, so a transform means what it reads like") {
    ...
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, 2 + 3 * 4, yPos, zPos);"), 0, 0, 0).x == 14);
    // Parentheses override it.
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, (2 + 3) * 4, yPos, zPos);"), 0, 0, 0).x == 20);
    // Subtraction, which no ISA here has an instruction for: a - b is emitted as a + (b * -1).
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, 100 - 40, yPos, zPos);"), 0, 0, 0).x == 60);
    // Left-associative, so 100 - 40 - 20 is 40 rather than 80.
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, 100 - 40 - 20, yPos, zPos);"), 0, 0, 0).x == 40);
    // The coordinate inputs compose with all of it — this is the shape a real modifier uses.
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, xPos * 2 + 1, yPos, zPos);"), 10, 0, 0).x == 21);
}
...
TEST_CASE("a scaled mirror, the transform this binding exists to make possible") {
    ...
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, (255 - xPos) * 2, yPos, zPos);"), 100, 5, 0);
...
TEST_CASE("editing a script asks the layer to rebuild its mapping") {
    ...
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, 7 - xPos, yPos, zPos);")));
    m.prepare();
...
TEST_CASE("the default script mirrors within the grid it is given, not a fixed 255") {
    ...
    const Coord3D p = transform(mmScriptAs("modifyLogical", "setXYZ(0, width - 1 - xPos, yPos, zPos);"), 0, 0, 0, /*w=*/16, /*h=*/16, /*d=*/1);
    CHECK(p.x == 15);
    CHECK(p.y == 0);

    // Every coordinate has to stay inside the box, or the Layer discards it.
    for (lengthType i = 0; i < 16; i++) {
        const Coord3D q = transform(mmScriptAs("modifyLogical", "setXYZ(0, width - 1 - xPos, yPos, zPos);"), i, 0, 0, 16, 16, 1);
        CAPTURE(i);
        CHECK(q.x >= 0);
        CHECK(q.x < 16);
    }
...
TEST_CASE("a script can read the grid extent it is folding within") {
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, width, yPos, zPos);"),  0, 0, 0, 32, 16, 1).x == 32);
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, height, yPos, zPos);"), 0, 0, 0, 32, 16, 1).x == 16);
}
...
TEST_CASE("a script that computes a position outside the grid leaves lights mapped") {
    ...
    MoonLiveModifier m;
    m.defineControls();
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, xPos + 200, yPos, zPos);")));   // deliberately off the end of a 16-wide grid
    m.prepare();
    Coord3D box{16, 16, 1};
    m.modifyLogicalSize(box);
...
TEST_CASE("re-preparing with an unchanged script does not ask for another rebuild") {
    ...
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "setXYZ(0, yPos, xPos, zPos);")));
    m.prepare();
...
TEST_CASE("print reports a value without changing what the script computes") {
    ...
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, print(xPos), yPos, zPos);"), 7, 3, 0, 16, 16, 1).x == 7);
    // And it composes inside arithmetic.
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, print(width - 1 - xPos), yPos, zPos);"), 0, 0, 0, 16, 16, 1).x == 15);
}
...
TEST_CASE("a subtraction produces the whole value, not just its low byte") {
    ...
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, print(width - 1 - xPos), yPos, zPos);"), 0, 0, 0, 16, 16, 1).x == 15);
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, print(100 - 1), yPos, zPos);"), 0, 0, 0, 255, 255, 1).x == 99);
    CHECK(transform(mmScriptAs("modifyLogical", "setXYZ(0, print(5 - 5), yPos, zPos);"), 0, 0, 0, 255, 255, 1).x == 0);
}
...
TEST_CASE("a for loop runs its body once per step") {
    MoonLiveModifier m;
    m.defineControls();
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "for (i = 0; i < 4; i = i + 1) { print(i); } setXYZ(0, xPos, yPos, zPos);")));
    m.prepare();
...
TEST_CASE("a loop over an empty range runs its body no times") {
    ...
    MoonLiveModifier m;
    m.defineControls();
    m.setScript(mmWriteScript(mmScriptAs("modifyLogical", "for (i = 0; i < 0; i = i + 1) { print(99); } setXYZ(0, xPos, yPos, zPos);")));
    m.prepare();
