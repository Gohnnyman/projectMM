...
// Stub driver: records the buffer it was given on each loop call so we can inspect what reached the driver after each pipeline step.
...
// Simulate Scheduler::prepareTree() + one tick: walk the top-level modules calling applyState() then tick().
//
// The quiesce is what makes this synchronous. With the multicore render↔encode split engaged, Drivers::tick() hands the frame to the core-1 task and RETURNS, the drivers run there, so a caller that reads a driver's result straight after tick() would race the worker. On the device that is the point (core 0 goes back to rendering; the next frame's boundary waits). Here the test wants the frame observable, so it waits the worker out, the same wait Drivers itself does at the next boundary. Off the split this is a no-op and the ticks were already synchronous.
...
TEST_CASE("Toggle a single layout off then on: pipeline survives and renders again") {
    // Regression: disabling the only layout child crashed Drivers::tick with a null-pointer blendMap on the next tick, Layer::prepare bailed early on physicalCount==0 without dropping the old LUT, while Drivers reallocated its output buffer to 0 bytes. The fix tears down the LUT and buffer on empty so Drivers takes the no-LUT path cleanly.
    mm::Layouts layouts;
    mm::GridLayout grid;
    ...
    // Include a modifier so the LUT is actually populated (hasLUT == true); the crash only occurs in this path because the no-LUT path doesn't read the output buffer.
    ...
    // Disable the only layout child. Drivers must survive (no crash) and see an empty source, assert both the structural release AND that the driver actually received an empty buffer (catches a regression where a stale buffer would forward through to the driver after layer release).
