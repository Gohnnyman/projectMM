...
/// Pins the 241-LED disc's wiring order. The disc is soldered either from the center LED outward (MoonLight's order, the default) or from the outer 60-LED ring inward, and a layout that only knew one would light the wrong ring for the other. The coordinates are the same set either way; what `outside in` changes is which index lands on which ring.
...
TEST_CASE("Rings241 wires from the center outward by default") {
    ...
    // Every outer-ring light is farther from the center than every inner light: the outer sixty must be the last sixty, or the wiring order is not center-out.
...
TEST_CASE("outside in puts light 0 on the outer ring and the center LED last, rings unchanged inside") {
    ...
    // Walk the default's rings from the last (outer) to the first (center) and expect them to appear in `out` in that order, each ring's own LED order intact.
...
// `angleFirst` says where light 0 of each ring sits, in degrees from the bottom. A disc is soldered with its first LED wherever the builder started, so without it the image is fixed against the hardware. The default of 0 must reproduce MoonLight's placement exactly, since that is what every other test here pins.
TEST_CASE("angleFirst moves the first light around the ring, and 0 leaves the disc where it was") {
    ...
    // A quarter turn moves every ring's first light. The disc is symmetric, so the SET of coordinates barely changes; what moves is which index sits where, which is the point.
    ...
    // 359 is one degree short of a full turn, so the disc is nearly back where it started: the control stops at 359 because 360 IS 0 (the ring closes) and offering both invites the question of which one it is.
