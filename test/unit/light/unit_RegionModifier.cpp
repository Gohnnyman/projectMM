...
// RegionModifier carves the layer to a sub-rectangle of the physical box, given as percentages. modifyLogicalSize reports the region size; modifyLogical folds a PHYSICAL coord into region-local space (subtract the start offset) and rejects any physical light outside the region. Half-open [start, end): abutting regions tile.
...
// Fold a physical coord; returns whether it's inside the region, leaving region-local pos in p. `box` is the physical box; logical is the region size.
...
TEST_CASE("RegionModifier abutting regions tile exactly") {
    ...
    // The seam: physical pixel 63 belongs to the LEFT region (region-local 63), pixel 64 belongs to the RIGHT (region-local 0). Each is in exactly one region.
    mm::Coord3D p;
