...
TEST_CASE("splat conserves total brightness across sub-pixel positions") {
    ...
    for (int frac = 0; frac < 256; frac += 16) {
        ...
        // Integer weights round, so allow a few units of slack, but nothing like a doubling or a halving, which is what a weight bug produces.
...
// A strand is the degenerate case the dimension-generic rule promises: the same call works, and the axis with extent 1 simply has no second neighbor to share with.
...
TEST_CASE("a per-channel write never spills into the next light") {
    for (uint8_t cpl : {uint8_t{1}, uint8_t{2}}) {
        ...
        // Light 1 took the color it could hold...
