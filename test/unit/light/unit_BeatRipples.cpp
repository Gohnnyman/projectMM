...
/// The wave surface. A golden hash pins which pixels light, but it passes just as happily on a frame that is entirely black, which is how this effect once shipped rendering nothing at all: the stone pressed into the surface was scaled in units of tens while the render reads a slope in units of thousands, so every ripple fell below the threshold that lights a pixel. What is pinned here is that water is VISIBLE and that it MOVES.
...
TEST_CASE("still water shows its ripples, and they spread") {
    ...
    // With no audio at all the idle rain is the only source, and the first drop lands on the opening frame rather than after a silent interval of black water.
    ...
    // And the surface keeps ringing rather than settling immediately: a wave that vanished in a frame would satisfy the check above while still looking like nothing.
