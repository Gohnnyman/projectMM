...
const ff=async p=>{const r=await fetch('/api/file?path='+encodeURIComponent(p));if(!r.ok)throw Error(p+': HTTP '+r.status);return r.text()};
...
const files={};
const skipped=[];const walk=async d=>{for(const e of await fd(d)){const p=(d==='/'?'':d)+'/'+e.name;if(e.isDir)await walk(p);else{let t;try{t=await ff(p)}catch(_){skipped.push(p);continue}const n=new TextEncoder().encode(t).length;if(n<e.size)throw Error(p+': got '+n+' of '+e.size+' bytes (truncated read)');if(n>e.size)skipped.push(p);else{files[p]=t;ui.textContent='projectMM backup: '+Object.keys(files).length+' files…'}}}};
await walk('/');
